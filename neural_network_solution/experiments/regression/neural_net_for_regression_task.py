...
def build_regression_model2(x_train, columns_to_check):
    input_models=[]
    output_embeddings=[]
...
def build_regression_model1(x_train, columns_to_check):
    input_models=[]
    output_embeddings=[]
    for c in columns_to_check:
      cat_emb_name= c +'_Embedding'
...
def train_regression_model(model, x_train, y_train, columns_to_check, plot_loss=False):
